...
class BorderScanner(BioTasker):
    ...
    def retriveR2BackgroundForward(self, startR2, endR2, refR2DiGraph):
        ...
        while (current and current <= endR2):
            ...
            #current = list(refR2DiGraph.successors(current))[0]
            succs = list(refR2DiGraph.successors(current))
            if (succs):
                current = succs[0]
            else:
    ...
    def estimateBorder(self, posSignalDict, backgroundDict):
        ...
        bRugSignals = [random.uniform(pos-0.5, pos+0.5) for pos, val in backgroundDict.items() for _ in range(0, val)]
        try:
    ...
    def runTask(self, chromNode):
        ...
        components = list(networkx.connected_components(testG.to_undirected()))
